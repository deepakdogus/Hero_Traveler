...
class RNDJDraftJSEditText(val context: ReactContext, var tag: Int) : EditText(context) {
    ...
    private val filter = DraftJsInputFilter(
            { OnInsertTextRequest(tag, it).sendIf(onInsertTextEnabled) },
            { OnBackspaceRequest(tag).sendIf(onBackspaceEnabled) },
            { OnNewlineRequest(tag).sendIf(onNewlineEnabled) },
    ...
    private fun setIntrinsicContentSize() {
        ...
        uiManager.setViewLocalData(tag, localData)
    }
    ...
    private fun manageSpans(spannableStringBuilder: SpannableStringBuilder) {
        ...
        for (span in spans) {
            ...
            when (span) {
                ...
                is StyleSpan,
                is AlignmentSpan,
                is ReactTagSpan -> text.removeSpan(span)
            }

            if (text.getSpanFlags(span) and Spanned.SPAN_EXCLUSIVE_EXCLUSIVE != Spanned.SPAN_EXCLUSIVE_EXCLUSIVE) {
                continue
            }
    ...
    override fun requestFocus(direction: Int, previouslyFocusedRect: Rect?): Boolean {
        return if (blockFocusRequest && !hasFocus()) {
            OnSelectionChangeRequest(tag, _selection.copy(hasFocus = true)).sendIf(onSelectionChangedEnabled)
            false
        } else {
    ...
    override fun onFocusChanged(focused: Boolean, direction: Int, previouslyFocusedRect: Rect?) {
        ...
        if (!focused) {
            ...
            OnSelectionChangeRequest(tag, _selection).sendIf(onSelectionChangedEnabled)
        }
    }
