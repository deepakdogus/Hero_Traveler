...
const hasInitialAppDataLoaded = ({ entities }, userId) =>
  isInitialAppDataLoaded(entities.users, userId)
const isStoryLikedSelector = ({ entities }, userId, storyId) =>
  isStoryLiked(entities.users, userId, storyId)
const isStoryBookmarkedSelector = ({ entities }, userId, storyId) =>
  isStoryBookmarked(entities.users, userId, storyId)
...
function* getInitalData(api, userId) {
  ...
  if (!initialAppDataLoaded) {
    const [likesResponse, bookmarksResponse, followingResponse] = yield all([
...
export function* getBookmarks(api, { userId }) {
  const response = yield call(api.getBookmarks, userId)
...
function* uploadAtomicAssets(draft) {
  ...
  const promise = yield Promise.all(
    draft.draftjsContent.blocks.map((block, index) => {
            ...
            .then(response => {
              ...
              const responseData =
                typeof response.data === 'string'
                  ...
                  : response.data
              if (
                responseData.resource_type === 'video' &&
                url &&
                draft.id &&
...
function getAtomicSteps(story) {
  ...
  return story.draftjsContent.blocks.reduce((count, block) => {
    if (block.type === 'atomic' && block.data.url.substring(0, 4) === 'file')
      return count + 1
    return count
  }, 0)
}
...
export function* flagStory(api, { userId, storyId }) {
  const response = yield call(api.flagStory, storyId)
  if (response.ok) {
...
export function* loadDrafts(api) {
  const response = yield call(api.getDrafts)
...
export function* getGuideStories(api, { guideId }) {
  ...
  // no fail case... worse case they will see fewer stories
}

export function* deleteStory(api, { userId, storyId }) {
  const response = yield call(api.deleteStory, storyId)
